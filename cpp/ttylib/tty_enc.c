...
Word16 tty_enc(
    ...
    Word16   *tty_baud_rate, /* (i/o): tty baud rate                     */
    float    pcm_buf_f[],    /* (i): input pcm                           */
    Word16   len             /* (i): length of pcm buffer (FRAMESIZE)    */
)
{
    Word16  pcm_buf [FRAMESIZE];
    Word16  i;

    for( i=0 ; i < len ; i++ )
    {
        pcm_buf[i] = (INT16)pcm_buf_f[i];                       
    }

    tty_header_in( counter );

    /* Save history dits */
    for( i=0 ; i < DITBUF_LEN-DITS_PER_FRAME ; i++ )
    {
        ditbuf[i] = ditbuf[DITS_PER_FRAME+i];       
    }

    baudot_to_dit( ditbuf+DITBUF_LEN-DITS_PER_FRAME,
                   pcm_buf );


    dit_to_ascii( tty_char, counter, tty_baud_rate, ditbuf);
