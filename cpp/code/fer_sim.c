	...
	for(k=0;k<NSTATES;k++) {
		for(i=0;i<NRATES_IN;i++) {
			...
			if( x > 1e-5) {
                ...
                fprintf(stdout,"Probabilities do not sum to one, rate in = %d, state = %d\n",i,k);
				return(0);
			}
		}
	}
...
void fer_sim(short *rate)
{
    ...
    if( first_time )
    {
        ...
        {
            fprintf(stdout,"FER Transition Prob. File              : \"%s\"\n\n",trans_fname);
        }
	
        /* record initial parameters in log file */
        fprintf(stdout,"Random number generator initial seed : %d\n",fer_sim_seed);
            ...
            for(j=0;j<NRATES_IN;j++)
            {
                for(k=0;k<NRATES_OUT;k++)
                {
                    ...
                    if(ix!=1)
                    {
                        ...
                        fprintf(stdout,"Error in read of transition matrices file\n");
                        exit(1);
                    }
                }
            }
        ...
        if(!prob_to_cum())
        {
            ...
            fprintf(stdout,"Problem with prob_to_cum()\n");
        }
    ...
    else if (rate_in == -2)
    {
        ...
        fprintf(stdout,"Encoded data file format error -- invalid rate specified in %d-th frame\n",frame_cnt);
    }
