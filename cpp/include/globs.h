...
#include  "macro.h"
extern float   Scratch[SubFrameSize + 6];
extern short   SScratch[6];     /* scratch short memory */

extern float   lsp[ORDER];      /* Correlation coefficients                */
extern float   lspi[ORDER];     /* Interpolation of correlation coeff      */

extern float   pci[ORDER];      /* Interpolated prediction coefficients    */
extern float  *gnvq;            /* Quantization table for fcb gain */
...
extern int     bit_rate;        /* Current bitrate for encoder/decoder */

extern int     FCBGainSize; /* Current Fixed Codebook Gain Size */
extern float    delay;          /* current frames delay */
extern short    LPCflag;        
extern int      fcbIndexVector[10];
